...
//
//    If nastiness>0, then some corruption may be (intentionally
//    introduced on output, or false data returned occasionally
//    from reads.) Nastiness levels 1-3 are deterministic:
//    if you do call nastytestfile on the same input
//    directory with the same contents twice, the results should 
//    be the same, but only if the whole directory is unmodified.
//    Nastiness levels 4-5 have a degree of randomness, and
//    will likely produce different results from one run
//    to the next (as is the case with the real errors
//    we're simulating).
//
